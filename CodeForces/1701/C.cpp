...
void solve() {
    ...
    f0r(i, n) { tasks.insert(a[i]); }
    while (true) {
        int last = *(tasks.rbegin());
        int first = *(tasks.begin());
        if (last - 1 >= first + 2) {
            tasks.extract(last);
            tasks.extract(tasks.begin());
            tasks.insert(first + 2);
            tasks.insert(last - 1);
            dbgs cout << first << ", " << last << endl;
        } else
            break;
    }
    cout << *(tasks.rbegin()) << endl;
}
