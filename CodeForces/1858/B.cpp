...
void solve() {
    ...
    dp[0][2] = 1;
    f1r(i, m) {
        auto stepPrev = step(s[i - 1], s[i] - 1, d);
        auto step2Prev = i > 1 ? step(s[i - 2], s[i] - 1, d) : 0;
        dp[i][0] = dp[i - 1][0] + 1 + stepPrev;
        dp[i][1] = max(dp[i - 1][2] + stepPrev + 2, step2Prev);
        dp[i][2] = stepPrev;
    }
    auto ans = dp[m - 1][0] - dp[m - 1][2];
}
