...
optional<pii> give_take(ll num) {
    ...
    if (num != 0) return optional<pii>();
    return reverse ? pii{n, m + n} : pii{m + n, n};
}

ostream& operator<<(ostream& os, pii p) {
    return os << "{" << p.first << ", " << p.second << "}";
}
ostream& operator<<(ostream& os, optional<pii> p) {
    if (p.has_value()) return os << p.value();
    return os << "{x, x}";
}
...
int main() {
    MAIN_FILE_HEADER
    for (int i = -20; i < 21; i++) { cout << i << ": " << give_take(i) << endl; }
    return 0;
}