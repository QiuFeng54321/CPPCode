...
#pragma endregion
typedef vector<set<int>> arr_t;

int n;
void rm_single(arr_t& arr) {
    int k = 0;
    f0r(i, 9) {
        if (arr[i].size() == 1) k++;
    }
...
void rm_except(int idx, arr_t& arr) {
    int k = 0;
    f0r(i, 9) {
        if (i != idx) k += arr[i].size();
    }
    cout << "- " << k << " ";
    f0r(i, 9) {
        if (i == idx) continue;
        for (const auto& v : arr[i]) {
            cout << (v + 1) << " ";
        }
    }
    cout << endl;
...
void solve() {
    ...
    rm_single(a);
    while (true) {
        arr_t newArr(9, set<int>());
        read_arr(newArr);
        int foundGroup = -1;
        f0r(i, 9) {
            if (newArr[i].size() == a[i].size() + 1) {
                foundGroup = i;
                break;
            }
        }
        ...
        if (newArr[foundGroup].size() == 1) {
            cout << "! " << (*newArr[foundGroup].begin() + 1) << endl;
            return;
        }
        rm_except(foundGroup, newArr);
        a = newArr;
    }
}
