...
const int N = 1e6;
ll dp[2 * N];
int t, n, nextLevelWidth = 2, lastOffset = 0;
vector<ll> p[1414];
...
void solve() {
    ...
    while (nextLevelWidth * (nextLevelWidth - 1) / 2 - 1 <= n) {
        auto curOffset = lastOffset + nextLevelWidth - 1;
        f0r(i, nextLevelWidth) {
            auto curIdx = curOffset + i;
            auto square = (ll)(curIdx + 1) * (curIdx + 1);
            if (i < nextLevelWidth - 1) dp[curIdx] += dp[lastOffset + i];
            dp[curIdx] += square;
            auto pi = nextLevelWidth - i - 1;
            if (i) {
                dp[curIdx] += p[pi].back();
                p[pi].push_back(p[pi].back() + square);
            } else {
                p[pi].push_back(square);
            }
        }
