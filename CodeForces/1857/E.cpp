...
void solve() {
    ...
    f0r(i, p) {
        m[x[idx[i]]] = i;
        dbgs cout << "m[" << x[idx[i]] << "] = " << i << endl;
    }
    vector<ll> rpsum(p), psum(p), dup(p);
    rpsum[p - 1] = 1;
    psum[0] = 1;
    for (int i = 0; i < p; i++) {
        dup[i] ++;
        if (i != 0)
            psum[i] = psum[i - 1] + (i) * (x[idx[i]] - x[idx[i - 1]]) + 1;
        if (i != p - 1) {
            int j = p - i - 2;
            rpsum[j] = rpsum[j + 1] + (i + 1) * (x[idx[j + 1]] - x[idx[j]]) + 1;
        }
    }
    dbgs {
        cout << "Psums" << endl;
    ...
    for (int i = 0; i < p; i++) {
        cout << (psum[m[x[i]]] + rpsum[m[x[i]]] - dup[m[x[i]]]) << " ";
    }
    cout << endl;
}
